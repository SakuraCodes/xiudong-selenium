...
def load_driver():
    ...
    driver.execute_cdp_cmd(
        "Page.addScriptToEvaluateOnNewDocument", {"source": inject_js}
    )
...
def goto_login_url(driver: webdriver.Chrome):
    ...
    WebDriverWait(driver, 100).until(EC.title_is("我的"))  # 最多等待 100s
...
def confirm_ticket(payBtn):
    ...
    while True:
        try:
            ...
            print(
                f"点击支付按钮发生异常，可能是已经抢票成功, 请查看手机 但是先不要退出 {e}"
            )
            continue
...
def select_people(driver, num: int):
    ...
    confirm = WebDriverWait(driver, 3).until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, ".pop-box>.pop-head>uni-view:nth-child(2)")
        )
    )
    confirm.click()
...
def create_instance(
    ...
    confirm_url = (
        f"https://wap.showstart.com/pages/order/activity/confirm/confirm"
